...
class model
{
	...
	public function getCommunityByAddress($address1,$address2){
		$url = $this->obapiurl ."/propertyapi/v1.0.0/property/detail?address1=$address1&address2=$address2";
		return $this->curlPOIAPI($url);  
	}
	
	public function getAreaHierarchy($lat,$long,$areaType){
		$url = $this->obapiurl . "/areaapi/v2.0.0/hierarchy/lookup?WKTString=POINT(" . $long . "," . $lat . ")&geoType=".$areaType;
		return $this->curlPOIAPI($url);  		
	}
	
	public function getAreaBoundary($areaid){
